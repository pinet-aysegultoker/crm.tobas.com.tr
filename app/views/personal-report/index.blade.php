...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-crm">
                    <div class="panel-heading"><h1><i class="fa fa-eye" aria-hidden="true"></i>Kişisel Satışları
                            Görüntüle</h1></div>
                                ...
                                @foreach($sales as $sale)
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="{{ route('customer.show', $customer->id) }}"
                                               target="_blank">{{ $customer->first_name }}
                                                <b>{{ $customer->last_name }}</b></a></td>
                                        <td><a href="{{ route('apartment.show', $apartment->id) }}"
                                               target="_blank">{{ $project->title }} > {{ $building->title }}
                                                > {{ $apartment->number }}</a></td>
                                        <td><a href="{{ route('pdf.offer.show', $offer->id) }}">{{ $offer->id }}
                                                -{{ Carbon::parse($offer->created_at)->formatLocalized('%d%m%Y-%H%M%S') }}
                                                -{{ $user->id }}</a></td>
                                        <td>{{ Carbon::parse($sale->created_at)->formatLocalized('%d/%m/%Y %H:%M:%S') }}</td>
                                        <td>
                                            <div class="btn-group btn-group-justified" role="group">
                                                <div class="btn-group" role="group">
                                                    <a type="button" class="btn btn-xs btn-info" href="#" role="button"><span
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-crm">
                    <div class="panel-heading"><h1><i class="fa fa-eye" aria-hidden="true"></i>Kişisel Teklifleri
                            Görüntüle</h1></div>
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-hover table-crm">
                                ...
                                @foreach($offers as $offer)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ Carbon::parse($offer->start_date)->formatLocalized('%d/%m/%Y') }}
                                            - {{ Carbon::parse($offer->finish_date)->formatLocalized('%d/%m/%Y') }}</td>
                                        <td>{{ Carbon::parse($offer->created_at)->formatLocalized('%d/%m/%Y %H:%M:%S') }}</td>
                                        <td>
                                            <div class="btn-group btn-group-justified" role="group">
                                                <div class="btn-group" role="group">
                                                    <a type="button" class="btn btn-xs btn-info"
                                                       href="{{ route('pdf.offer.show', $offer->id) }}"
                                                       role="button"><span class="fa fa-file-pdf-o"
                                                                           aria-hidden="true"></span></a>
                                                </div>
                                                <div class="btn-group" role="group">
                                                    <button type="button" class="btn btn-xs btn-success"
                                                            data-toggle="modal" data-target="#removeModal"
                                                            data-customer="{{$offer->customer_id}}"
                                                            data-id="{{$offer->id}}"
                                                            data-creatorid="{{$offer->creator_id}}"
                                                            data-apartmentid="{{$offer->apartment_id}}"
                                                            data-isim="{{$customer->first_name}} {{$customer->last_name}}">
                                                        <span class="fa fa-check-square-o"
                                                              aria-hidden="true">Satış</span>
                                                    </button>
                                                </div>
                                            </div>
                                        </td>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-crm">
                    <div class="panel-heading"><h1><i class="fa fa-eye" aria-hidden="true"></i>Kişisel Günlük
                            İşlem Raporu</h1></div>
                    <div class="panel-body">
                        <div class="row">
                        ...
                        @forelse($processes as $process)
                            <div class="row" style="background-color: #fff;">
                                <!--Foreach Start-->
                                <div class="col-md-12 text-center" style="border-bottom: 1px solid #111;">
                                    <h4 style="font-weight: 700">İşlem Yapılan Tarih</h4>
                                    <h5>{{date('d-m-y', strtotime($process->date))}}</h5>
                                </div>
                                <div class="col-md-4 col-md-offset-2 text-center">
                                    <h5 style="border-bottom: 1px solid #111; padding-bottom: 4px;font-weight: 700;">
                                        İşlem Yapan Kişi</h5>
                                    <p>{{$process->name}}</p>
                                </div>
                                <div class="col-md-4 text-center">
                                    <h5 style="border-bottom: 1px solid #111; padding-bottom: 4px;font-weight: 700;">
                                        Gerçekleştirdiği İşlem Sayısı</h5>
                                    <p>{{$process->proces_number}}</p>
                                </div>
                                <!--Foreach end-->
                            </div>
                        @empty
